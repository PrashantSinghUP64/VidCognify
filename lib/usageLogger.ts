 ...
 * @param service - The API service being used (e.g., 'groq')
...
export async function getUserUsageStats(
  ...
  const byService = logs.reduce((acc: Record<string, { totalRequests: number; totalCredits: number; totalTokens: number }>, log: { service: string; creditsUsed: number; tokensUsed: number }) => {
    if (!acc[log.service]) {
      acc[log.service] = {
        totalRequests: 0,
        totalCredits: 0,
  ...
  return {
    ...
    totalCredits: logs.reduce((sum: number, log: { creditsUsed: number }) => sum + log.creditsUsed, 0),
    totalTokens: logs.reduce((sum: number, log: { tokensUsed: number }) => sum + log.tokensUsed, 0),
  };
}
