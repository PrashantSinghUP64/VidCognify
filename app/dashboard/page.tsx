    ...
    <div className="min-h-screen gradient-soft p-4 md:p-8">
      <div className="max-w-5xl mx-auto space-y-8">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-slate-500 mt-2">Track your learning progress and analytics.</p>
        </motion.div>
        ...
        >
          <motion.div variants={itemVariants} className="card-soft p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{data.totalSummaries}</h3>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="card-soft p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{data.currentStreak} Days</h3>
            </div>
          </motion.div>
          ...
          <motion.div variants={itemVariants} className="card-soft p-6 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-2xl font-bold text-slate-900">{topCategory}</h3>
            </div>
          </motion.div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
           ...
           <motion.div className="card-elevated p-6" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
              <h2 className="text-lg font-semibold text-slate-800 mb-6">Weekly Activity</h2>
                ...
                {data.weeklyData.map((day, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center flex-1 gap-2 group">
                       ...
                       <div className="w-full bg-slate-100 rounded-t-md relative hover:bg-slate-200 transition-colors cursor-pointer flex-1 flex items-end">
                         <motion.div 
           ...
           <motion.div className="card-elevated p-6" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }}>
              <h2 className="text-lg font-semibold text-slate-800 mb-6">Category Distribution</h2>
              <div className="space-y-4">
                   ...
                   return (
                     <div key={cat}>
                       <div className="flex justify-between text-sm mb-1 text-slate-600">
                         <span className="font-medium inline-flex items-center gap-2 text-slate-700">🏷️ {cat}</span>
                 ...
                 {Object.keys(data.categoryCounts).length === 0 && (
                   <div className="text-center text-slate-500 py-10 text-sm">No data available</div>
                 )}
              </div>
           </motion.div>
        </div>
