...
import { extractMetrics, type ExtractedMetrics } from "@/lib/metricsExtraction";
 ...
 */
type Stage = "fetching_transcript" | "analyzing_topics" | "generating_summary" | "building_timeline" | "analyzing_metrics";
...
export async function POST(req: NextRequest) {
  ...
  (async () => {
    try {
      ...
      if (existingSummary) {
        await writeProgress({
          ...
          summary: {
            ...
            transcriptSegments: existingSummary.transcriptSegments.map((s: { id: string; text: string; offset: number; duration: number; order: number }) => ({
              id: s.id,
              text: s.text,
              offset: s.offset,
              duration: s.duration,
            ...
            modelUsed: "llama-3.3-70b-versatile", // Default for cached results
            source: "cache",
          },
          status: "completed",
        });
      ...
      let metrics: ExtractedMetrics | null = null;
      await writeProgress({
        type: "progress",
        stage: "analyzing_metrics",
        message: "Analyzing sentiment and difficulty...",
      });

      try {
        const metricsResult = await extractMetrics(transcriptText, summary, { userId });
        metrics = metricsResult.metrics;

        // Log usage
        await logApiUsage(
          userId,
          metricsResult.modelUsed,
          "metrics_extraction",
          0,
          metricsResult.tokensUsed || 0
        );
      } catch (metricsError) {
        console.warn("Metrics extraction failed:", metricsError);
      }
      ...
      await writeProgress({
        ...
        summary: {
          ...
          transcriptSegments: savedSummary.transcriptSegments.map((s: { id: string; text: string; offset: number; duration: number; order: number }) => ({
            id: s.id,
            text: s.text,
            offset: s.offset,
            duration: s.duration,
